...
def generate_qr(url, filename, fill_color, back_color, logo_file, d_color):
    ...
    if os.path.exists(logo_file):
        ...
        # D logo shape size (keeps readability at 50% of QR size)
        d_size = int(1290 * 0.50)
...
def generate_qr_svg(url, filename, fill_color, back_color, d_color):
    ...
    svg_parts.append('      <stop offset="40%" stop-color="#fcf3da" stop-opacity="1" />')
    ...
    # Logo Group (Decoupled circle size: 60% circle, 50% D)
    circle_scale = 0.60
    d_scale = 0.50
...
def main():
    ...
    print("\n--- Generating Home QR Codes ---")
    for color_name, palette in COLORS.items():
        filename_png = os.path.join(OUTPUT_DIRS["home"], f"QUEENS_HOME_QR_{color_name}.png")
